<?
    ...
    for ($j = 0; $j < $numDays; $j++) {
    	...
    	$date["date"] = date('m-j, l', strtotime($datesarray[$j]));
    	
    	// put the date formatted for sql in the array too!
    	$date["datesql"] = $datesarray[$j];
    	
    	// prepare an array for the occupancy
    	$date["rooms"] = array();
    	...
    	// if result, then put the result in the right place
    		...
    		for ($k = 0; $k < 7; $k++) {
    			if ($k + 1 == $row_room["room"]) {
    				$date["rooms"][$k] = $row_room["transactionid"];
    				$row_room = mysql_fetch_assoc($result_room);
    			}
    			else
    				$date["rooms"][$k] = "Vacant";
